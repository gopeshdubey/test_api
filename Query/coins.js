      ...
      if (symbol_data.length > 0) {
        ...
        if (total <= balance) {
          ...
          if (updated_data.nModified > 0 && open_order.nModified > 0) {
            ...
            // GET HIGHEST BID PRICE OF BUY IN COIN TABLE
            // IF
            // HIGHEST BID DOESN'T EXIST SO END OPERATION
            // ELSE
            // CHECK HIGHEST BID IS EQUAL TO BIDDED PRICE
            // IF FALSE SO END OPERATION
            // ELSE
            // CHECK AMOUNT EQUAL TO BIDDED AMOUNT
            var highest_bid = await get_lowest_sell_bid_from_coin_table(
              symbol,
              pair_with
            );
...
const get_last_buy_bid_in_coin_table = (symbol, price, pair_with) => {
  return new Promise((res, rej) => {
    Coins.aggregate(
      [
        { $unwind: "$last_price" },
...
const get_sorted_user_data_in_last_price_in_coin_table_for_sell_last_price_id = (
  last_price_id,
  symbol,
  pair_with
) => {
...
const get_lowest_sell_bid_from_coin_table = (symbol, pair_with) => {
  return new Promise((res, rej) => {
    Coins.aggregate(
      [
        { $unwind: "$last_price" },
    ...
    if (check_user_data_array.length > 0) {
      if (check_user_data_array[0].last_price[0].user_data.length == 0) {
        ...
        if(check_user_data_array.length > 0) {
          ...
          var last_price_price = await check_user_data_array[0].last_price[0].price,
...
const get_highest_buy_bid_from_coin_table = (symbol, pair_with) => {
  return new Promise((res, rej) => {
    Coins.aggregate(
      [
        { $unwind: "$last_price" },
